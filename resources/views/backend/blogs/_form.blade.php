<div class="w-full  bg-gray-50 px-12 py-12">
    <input type="text" placeholder="Write your title" name="title" value="{{isset($blog->title) ? $blog->title :''}}" class="my-3 w-full border-gray-300" />
    @error('title')
        <div class="text-red-600">{{ $message }}</div>
    @enderror
    <textarea name="dis" placeholder="Discription" class="w-full border-gray-300">{{isset($blog->dis) ? $blog->dis : ''}}</textarea>
    @error('dis')
        <div class="text-red-600">{{ $message }}</div>
    @enderror
    <div class="w-full my-4">
        ...
        <input
            ...
            <img src="{{isset($blog->images) ? asset('storage/'. $blog->images) : ''}}" alt="" srcset="">
        @error('images')
            <div class="text-red-600">{{ $message }}</div>
        @enderror
    </div>
